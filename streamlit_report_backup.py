...
# Check if waiver wire needs to be run (daily at 8am)
def check_and_run_daily_waiver():
    """Run waiver wire analysis if it hasn't been run today before 8am"""
    ...
    if should_run:
        st.sidebar.info("🔄 Running scheduled waiver wire analysis...")
        try:
            subprocess.run(
            ...
            st.sidebar.success("✅ Daily waiver wire analysis complete!")
        except subprocess.CalledProcessError as e:
            st.sidebar.error(f"❌ Waiver wire analysis failed: {e}")
